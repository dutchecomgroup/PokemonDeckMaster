...
const CollectionStats: React.FC = () => {
  const { collections, getCollectionCards } = useCollectionContext();
  ...
  // Set default selected collection when collections are loaded
  useEffect(() => {
  ...
  const collectionCards = useMemo(() => {
    ...
    // Get cards for the selected collection
    const cards = getCollectionCards(selectedCollectionId.toString());
    
    // Map card IDs to actual card objects
    return cards.map(cardId => {
      return {
        id: cardId,
        // Other card properties will be filled from the stored cards
        // This is just a placeholder structure until we get the full card data
      };
    });
  }, [getCollectionCards, selectedCollectionId]);
  
  // Get set IDs from the collection cards
  const setIds = useMemo(() => {
    return Array.from(new Set(collectionCards.map(card => 
      typeof card === 'object' && card.set?.id || null).filter(Boolean)));
  }, [collectionCards]);
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Create a collection first to view statistics
            </CardDescription>
          </CardHeader>
          <CardContent className="h-64 flex items-center justify-center">
            <div className="text-center text-muted-foreground">
              <p>You need to create a collection before viewing statistics</p>
              <p className="text-sm mt-2">Go to the Collection Manager to get started</p>
            </div>
          </CardContent>
        </Card>
      </div>
    ...
    <div className="space-y-6 pb-8">
      ...
      {selectedCollection && (
        <div className="mb-6">
          ...
          <p className="text-muted-foreground text-sm">{selectedCollection.description || 'No description available'}</p>
        </div>
      )}
