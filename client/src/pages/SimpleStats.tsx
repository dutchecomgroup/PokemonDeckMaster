    ...
    return (
      <div className="bg-popover border border-border rounded-md shadow-md p-2 text-xs">
        ...
        {payload[0].percent !== undefined && (
          <p className="text-muted-foreground">{`${Math.round(payload[0].percent * 100)}%`}</p>
        )}
      </div>
    );
  }
...
const SimpleStats: React.FC = () => {
  ...
  // Set first collection as default when collections load
  useEffect(() => {
  ...
  // Get cards for the current collection
  const collectionCards = useMemo(() => {
    if (!selectedCollectionId) return [];
    
    return getAllCollectionCards().filter(card => {
      // Check if card belongs to the selected collection
      // Note: We need to check both direct collectionId and array of collectionIds
      if (typeof card.collectionId === 'number') {
  ...
  // Get set IDs from collection cards
  const setIds = useMemo(() => {
    return Array.from(new Set(collectionCards
      .map(card => card.set?.id)
      .filter(Boolean)));
  }, [collectionCards]);
  
  // Fetch set cards for completion calculation
  const { data: allSetCards = [] } = useQuery({
    queryKey: ['/api/sets', setIds],
    queryFn: async () => {
  ...
  // Generate statistics from collection cards
  useEffect(() => {
    if (collectionCards.length === 0) return;
    
    // Get unique cards
    const uniqueCardIds = new Set(collectionCards.map(card => card.id));
    ...
    // Create simulated historical value data
    const now = new Date();
    for (let i = 5; i >= 0; i--) {
    ...
    uniqueCards.forEach(card => {
      ...
      // Create historical data with slight growth pattern
    ...
    Object.entries(setCardCounts).forEach(([setId, { total, owned }]) => {
      const setInfo = allSetCards.find(card => card.set?.id === setId)?.set;
      if (!setInfo) return;
  ...
  // Render loading state if no collections
  ...
  // Render empty state if no cards in collection
        ...
        <Card>
          ...
          <CardContent className="h-64 flex items-center justify-center">
            <div className="text-center text-muted-foreground">
              ...
              <p className="text-sm mt-2">Add cards via the Set Browser or Search</p>
            </div>
          </CardContent>
        </Card>
      </div>
