...
const StatisticsPage: React.FC = () => {
  // Local state for collection selection (no navigation)
  ...
  // Get all cards or filter by selected collection
  const collectionCards = useMemo(() => {
    ...
    // Filter to show only cards from the selected collection
    return allCards.filter((card: any) => card.collectionId === selectedCollectionId);
  }, [getAllCollectionCards, selectedCollectionId]);
  
  // Get sets from collections for completion data
  ...
  const { data: allSetCards = [] } = useQuery({
    queryKey: ['sets', 'all-cards', setIds],
    queryFn: async () => {
  ...
  // Process collection data to generate statistics
  useEffect(() => {
    ...
    <div className="space-y-6 pb-8">
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div 
            className={`cursor-pointer ${selectedCollectionId === null ? 'ring-2 ring-indigo-500' : ''}`}
            onClick={(e) => {
              e.preventDefault(); // Prevent any navigation
              setSelectedCollectionId(null);
            }}
          >
            <Card className="bg-gradient-to-br from-indigo-500/10 to-purple-500/10 border-indigo-200/30 hover:border-indigo-300/50 transition-all hover:shadow-md overflow-hidden relative">
          ...
          {collections.map(collection => (
            <div 
              key={collection.id}
              className={`cursor-pointer ${selectedCollectionId === collection.id ? 'ring-2 ring-purple-500' : ''}`}
              onClick={(e) => {
                e.preventDefault(); // Prevent any navigation
                setSelectedCollectionId(collection.id);
              }}
            >
              <Card className="bg-gradient-to-br from-purple-500/10 to-pink-500/10 border-purple-200/30 hover:border-purple-300/50 transition-all hover:shadow-md overflow-hidden relative">
